...
from neoninstall.system_operations import check_prerequisites, configure_system_settings
from neoninstall.disk_operations import select_disks, create_zfs_pool
...
from neoninstall.user_operations import setup_user, configure_ssh
...
def main():
    ...
    console.print("Please make sure you have a backup of any important data before proceeding.")
    ...
    if not configure_system_settings(pool_name):
        console.print("[bold red]Failed to configure system settings. Exiting.[/bold red]")
        return 1
    ...
    console.print("[bold yellow]Don't forget to change the password after first login![/bold yellow]")
