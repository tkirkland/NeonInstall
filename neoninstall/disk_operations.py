...
def select_disks() -> List[str]:
    ...
    if disks_with_filesystems and not handle_existing_filesystems(disks_with_filesystems):
        return []
...
def handle_existing_filesystems(disks_with_fs: List[str]) -> bool:
    ...
    """
    console.print("[bold yellow]Warning:[/bold yellow] The following disks have existing filesystems:")
...
"""
        ...
        console.print(f"[bold red]Error:[/bold red] Failed to create TRIM service files: {e}")
        return False
...
def create_zfs_pool(disks: List[str]) -> Tuple[bool, str, str]:
    ...
    pool_create_cmd = _build_pool_create_command(pool_type, pool_name, disks, zfs_partitions)

    try:
        subprocess.run(pool_create_cmd, check=True)
        console.print(f"[bold green]ZFS pool '{pool_name}' created successfully.[/bold green]")
...
def _get_available_pool_types(disk_count: int) -> List[Dict[str, str]]:
    ...
    if disk_count == 1:
        ...
        return [
            ...
            {"name": "Single disk (no redundancy, use second disk for separate pool)", "value": "single"}
        ]
    else:  # disk_count >= 3
        pool_types = [
            {"name": f"RAIDZ1 (RAID5, {disk_count - 1}/{disk_count} usable space)", "value": "raidz1"},
            {"name": "Mirror (RAID1, 50% usable space)", "value": "mirror"},
            {"name": "Single disk (no redundancy, use other disks for separate pools)", "value": "single"}
        ]
        if disk_count >= 4:
            pool_types.insert(1, {"name": f"RAIDZ2 (RAID6, {disk_count - 2}/{disk_count} usable space)", "value": "raidz2"})
        return pool_types
...
def _prepare_disk_partitions(disks: List[str]) -> Tuple[bool, str]:
    ...
    for disk in disks:
        try:
            ...
            console.print(f"[bold red]Error:[/bold red] Failed to create partitions on {disk}: {e}")
            return False, ""
...
def _build_pool_create_command(pool_type: str, pool_name: str, disks: List[str],
    ...
    if pool_type == "single":
        if len(disks) == 1:
            ...
            pool_create_cmd.extend([pool_name, zfs_partitions[0]])
            console.print("[bold yellow]Warning:[/bold yellow] Only using the first disk for the ZFS pool.")
...
def configure_pool_options(pool_name: str) -> bool:
    ...
    if not enable_pool_autotrim(pool_name):
        console.print("[bold yellow]Warning:[/bold yellow] Failed to enable autotrim, but continuing.")
        success = False

    if not _create_trim_service_files(pool_name):
        console.print("[bold yellow]Warning:[/bold yellow] Failed to create TRIM service files, but continuing.")
        success = False
