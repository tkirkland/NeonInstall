...
def _configure_sudo(pool_name: str) -> bool:
    ...
    Returns:
        bool: True if sudo configuration was successful, False otherwise
    """
    try:
        # Configure sudo
        ...
        subprocess.run(["chmod", "440", f"/{pool_name}/ROOT/etc/sudoers.d/me"], check=True)

        return True
    except (IOError, subprocess.CalledProcessError) as e:
        console.print(f"[bold red]Error:[/bold red] Failed to configure sudo: {e}")
        return False
...
def setup_user(pool_name: str) -> bool:
    ...
    try:
        ...
        if not _configure_sudo(pool_name):
            console.print("[bold yellow]Warning:[/bold yellow] Failed to configure sudo, but continuing with installation.")

        # Configure shell
        if not _configure_shell(pool_name):
            console.print("[bold yellow]Warning:[/bold yellow] Failed to configure shell, but continuing with installation.")
...
def _setup_ssh_keys(pool_name: str) -> bool:
    ...
    try:
        ...
        if ssh_key_option == "paste":
            ...
            subprocess.run([
                "ssh-keygen", "-t", "ed25519", "-f", f"/{pool_name}/ROOT/home/me/.ssh/id_ed25519",
                "-N", "", "-C", "me@precision"
            ], check=True)

            # Generate ECDSA key
            subprocess.run([
                "ssh-keygen", "-t", "ecdsa", "-f", f"/{pool_name}/ROOT/home/me/.ssh/id_ecdsa",
                "-N", "", "-C", "me@precision"
            ], check=True)

            # Generate RSA key
            subprocess.run([
                "ssh-keygen", "-t", "rsa", "-b", "4096", "-f", f"/{pool_name}/ROOT/home/me/.ssh/id_rsa",
                "-N", "", "-C", "me@precision"
            ], check=True)
...
"""
        ...
        console.print(f"[bold red]Error:[/bold red] Failed to write SSH configuration: {e}")
        return False
...
def configure_ssh(pool_name: str) -> bool:
    ...
    try:
        ...
        if not _write_sshd_config(pool_name):
            console.print("[bold yellow]Warning:[/bold yellow] Failed to write SSH configuration, but continuing with installation.")

        # Set up SSH keys
        if not _setup_ssh_keys(pool_name):
            console.print("[bold yellow]Warning:[/bold yellow] Failed to set up SSH keys, but continuing with installation.")
