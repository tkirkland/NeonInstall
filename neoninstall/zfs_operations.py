...
"""
        ...
        console.print(f"[bold red]Error:[/bold red] Failed to create snapshot service files: {e}")
        return False
...
def create_zfs_datasets(pool_name: str) -> bool:
    ...
    try:
        ...
        if not _create_snapshot_service_files(pool_name):
            console.print("[bold yellow]Warning:[/bold yellow] Failed to create snapshot service files, but continuing with installation.")
